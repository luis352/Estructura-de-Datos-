        ...
        private void btnmetodoordenamiento_Click(object sender, EventArgs e)
        {
            if (rbtAcendente.Checked)
            {
                ...
                MessageBox.Show("N° Comparaciones: " + Comparaciones +
                    ...
                    "\nDuracion del ordenado: " + MiliSegundos.ToString() + " ms", "METODO BURBUJA CON SEÑA{  (ASCENDENTE)");
            }
            ...
            if(rbtadecendente.Checked)
            {
                int Comparaciones = 0, Movimientos = 0, MiliSegundos = 0;
                DateTime tiempoInicio = DateTime.Now;
                Ordenamiento<Empleado>.BurbujaConSeñal(miEmpleadoArreglo, Ordenamiento<Empleado>.Descendente, out Comparaciones, out Movimientos, out MiliSegundos);
                MostrarDatos();
                MessageBox.Show("N° Comparaciones: " + Comparaciones +
                    "\nN° Intercambios: " + Movimientos +
                    "\nDuracion del ordenado: " + MiliSegundos.ToString() + " ms", "METODO BURBUJA CON SEÑAL (DECENDENTE)");
            }
